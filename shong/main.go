// Command shong is Shong for the PocketBook Verse Pro (PB634), built on the
// dennwc/inkview SDK.
//
// A lightning chess-like duel on a narrow 4x6 board. Each side starts with 4
// pieces on its own back rank: Triangel (diagonal moves), Kvadrat
// (orthogonal moves), X (all 8 directions) and Kung (exactly 1 step,
// alternating between the Triangel's and the Kvadrat's directions every time
// it itself moves). Triangel/Kvadrat/X start on a short move of 1 square;
// after a piece's first move it is "long" and always moves exactly 2 squares
// thereafter — an eye mark on the piece records this. No piece may jump; a
// move lands on an enemy piece by displacement capture. Win by capturing the
// enemy Kung, or by walking your own Kung to the opponent's back rank. Play
// hot-seat against a friend or against a built-in alpha-beta AI.
//
// Pure game logic (board, moves, captures, win conditions, AI) lives in the
// shong/game package with no SDK dependency and is unit-tested; this file
...
func (a *app) Draw() {
	...
	switch a.screen {
	case screenSplash:
		DrawSplash(screenSize, a.fonts, "Shong", drawSplashMotif)
		ink.FullUpdate()
	case screenMenu:
	...
	case screenRules:
		a.rulesBack = DrawRules(screenSize, a.fonts, "Shong", rulesParagraphs)
		ink.FullUpdate()
	}
}
...
func (a *app) tapGame(p image.Point) bool {
	...
	if a.hasSelection {
		...
		if piece := a.gs.Board.At(x, y); piece != nil && piece.Side == a.gs.Turn {
