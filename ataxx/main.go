// Command ataxx is Ataxx for the PocketBook Verse Pro (PB634), built on the
// dennwc/inkview SDK.
//
// Two players face off on a 7x7 board, each starting with 2 men in
// diagonally opposite corners. A move is either a CLONE (one step in any of
// the 8 directions — the original man stays put and a new man appears at the
// destination) or a JUMP (two steps away — the original vacates its cell and
// lands on the destination). Either way, every enemy man adjacent (all 8
// directions) to the destination flips to the mover's color. Play hot-seat
// against a friend or against a built-in alpha-beta AI. The game ends when
// the board fills or the side to move has no legal move (or no pieces left);
// whoever then has more men wins.
//
// Pure game logic (board, moves, flips, win condition, AI) lives in the
// ataxx/game package with no SDK dependency and is unit-tested; this file
...
func (a *app) Draw() {
	...
	switch a.screen {
	case screenSplash:
		DrawSplash(screenSize, a.fonts, "Ataxx", drawSplashMotif)
		ink.FullUpdate()
	case screenMenu:
	...
	case screenRules:
		a.rulesBack = DrawRules(screenSize, a.fonts, "Ataxx", rulesParagraphs)
		ink.FullUpdate()
	}
}
