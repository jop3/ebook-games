// Command bullscows is the Bulls and Cows digit-deduction game for the
...
// build a guess and submits it; feedback is given as Bulls (right digit, right
// place) and Cows (right digit, wrong place). Guess the code in as few tries as
// possible.
//
// Pure game logic (scoring, secret generation, entry state) lives in the
// bullscows/game package with no SDK dependency and is unit-tested; this file
...
func (a *app) Draw() {
	...
	switch a.screen {
	case screenSplash:
		DrawSplash(screenSize, a.fonts, "Bulls & Cows", drawSplashMotif)
		ink.FullUpdate()
	case screenMenu:
	...
	case screenRules:
		a.rulesBack = DrawRules(screenSize, a.fonts, "Bulls & Cows", rulesParagraphs)
		ink.FullUpdate()
	}
}
...
func (a *app) drawGame(screenSize image.Point) {
	...
	if a.gs.Solved || a.updates == 0 || a.updates%fullUpdateEvery == 0 {
		ink.FullUpdate()
	} else {
...
func (a *app) statusText() string {
	...
	return "Gissningar: " + itoa(len(s.Guesses)) + "   ·   " + itoa(s.Cfg.Length) + " olika siffror"
}
